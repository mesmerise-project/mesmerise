...
private fun grad(hash: Int, x: Double) = if(hash and 1 == 1) x else -x
...
class PerlinNoiseGen(
    val octaves : Int = 1,
    val persistence : Double = 1.0,
    val salt : Int = 1
) : NoiseGen1D {
    override fun saltedNoise(x: Double, salt : Int) =
        noise(x, this.octaves, this.persistence, salt)
    override fun noise(x : Double) =
        saltedNoise(x, this.salt)
}